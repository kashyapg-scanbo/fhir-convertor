...
describe('parseCustomJSON', () => {
  ...
  it('maps camelCase patient dynamic fields and custom patient extensions', () => {
    const input = {
      resourceType: 'Patient',
      patientFirstName: 'Bhavnaben',
      patientMiddleName: '',
      ...
      countryCode: null,
      gender: 'Female',
      birthDate: '1972-03-18T00:00:00.000+00:00',
      photo: null,
      age: null,
      weight: 69,
      weightUnit: 'kg',
    ...
    const canonical = parseCustomJSON(input);
    expect(canonical.patient?.name?.given?.[0]).toBe('Bhavnaben');
    expect(canonical.patient?.name?.family).toBe('Patel');
    expect(canonical.patient?.gender).toBe('Female');
    expect(canonical.patient?.birthDate).toBe('1972-03-18T00:00:00.000+00:00');
    expect(canonical.patient?.weight).toBe(69);
    expect(canonical.patient?.weightUnit).toBe('kg');
    expect(canonical.patient?.height).toBe(140);
    expect(canonical.patient?.heightTaken).toBe(false);
    expect(canonical.patient?.heightUnit).toBe('cm');
    expect(canonical.patient?.bloodGroup).toBe('AB-');
    expect(canonical.patient?.isPregnant).toBe(false);
    expect(canonical.patient?.isDiabetic).toBe(false);
