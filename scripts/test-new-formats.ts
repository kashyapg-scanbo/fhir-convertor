...
async function runTests() {
  ...
  const csvData = `MRN,First Name,Last Name,Gender,Birth Date
CSV-PAT-001,John,CsvDoe,Male,1985-05-05`;

  try {
    const r5BundleCsv = await convertLegacyData(csvData, 'csv');
    const patCsv = r5BundleCsv.entry?.find((e: any) => e.resource.resourceType === 'Patient')?.resource;

    if (patCsv.name[0].family === 'CsvDoe' && patCsv.identifier[0].value === 'CSV-PAT-001') {
      console.log('✅ CSV conversion passed');
    } else {
      console.error('❌ CSV conversion failed: Data mismatch', patCsv);
    }
  } catch (e) {
